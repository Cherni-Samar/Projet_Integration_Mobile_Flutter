...
import 'hr_inbox_screen.dart'; // ✅ IMPORT POUR LA BOÎTE DE RÉCEPTION
...
class _HrDashboardPageState extends State<HrDashboardPage>
  ...
  void initState() {
    ...
    _loadAdminData();
    _refreshTimer = Timer.periodic(const Duration(seconds: 30), (_) => _loadAdminData());
  }
  ...
  Future<void> _loadStats() async {
    ...
    try {
      ...
      if (result['success'] == true) {
        setState(() { _stats = result['stats']; _loadingStats = false; });
      } else {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // HEADER — avec bouton "Messages" et "Envoyer à Echo"
  // ══════════════════════════════════════════════════════════════════════════
        ...
        children: [
          ...
          Container(
            ...
            child: ClipRRect(
              ...
              child: Image.asset('assets/images/hera.png', fit: BoxFit.cover),
            ),
          ),
            ...
            child: IconButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => HrInboxScreen(token: null),
                  ),
                );
              },
              icon: const Icon(
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isSelected ? const Color(0xFFCCFF00).withOpacity(0.15) : Colors.transparent,
  ...
  Widget _buildHistorySection(bool isDark) {
    return Column(
      ...
      children: [
        ...
        // ✅ Dismissible avec index pour swipe-to-delete
          ...List.generate(_recentActions.length, (index) {
            ...
            switch (action['action_type']) {
              ...
              case 'leave_approved':
                icon = Icons.check_circle;
                color = const Color(0xFFCCFF00);
                final nameA = action['employee_name'] ?? 'Employé';
                text = 'Congé de $nameA approuvé automatiquement';
                break;
              case 'leave_refused':
                icon = Icons.cancel;
                color = const Color(0xFFEF4444);
                final nameR = action['employee_name'] ?? 'Employé';
                text = 'Congé de $nameR refusé';
                break;
              default:
                switch (action['status']) {
                  case 'approved':
                    icon = Icons.check_circle;
                    color = const Color(0xFFCCFF00);
                    final n = action['employee_name'] ?? 'Employé';
                    text = 'Congé de $n approuvé';
                    break;
                  case 'refused':
                    icon = Icons.cancel;
                    color = const Color(0xFFEF4444);
                    final n = action['employee_name'] ?? 'Employé';
                    text = 'Congé de $n refusé';
                    break;
                  default:
                    icon = Icons.info;
            ...
            return Dismissible(
              key: Key('action_${action['_id'] ?? index}_$index'),
              direction: DismissDirection.startToEnd,
              onDismissed: (_) => _removeAction(index),
          ...
          Container(
            ...
            child: TableCalendar(
                    ...
                    return Positioned(
                      ...
                      child: Row(
                        ...
                        children: events.take(3).map((event) {
                          ...
                          Color dotColor;
                          switch (leave['type']) {
                            case 'urgent': dotColor = const Color(0xFFEF4444); break;
                            case 'sick':   dotColor = const Color(0xFFF59E0B); break;
                            default:       dotColor = const Color(0xFFCCFF00);
                          }
                          return Container(
                            width: 5, height: 5,
              ...
              calendarStyle: CalendarStyle(
                ...
                defaultTextStyle: TextStyle(color: isDark ? Colors.white : Colors.black, fontSize: 14),
                weekendTextStyle: TextStyle(color: isDark ? Colors.white70 : Colors.black54, fontSize: 14),
              ...
              headerStyle: HeaderStyle(
                ...
                formatButtonTextStyle: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold),
  ...
  Widget _buildEnergy(bool isDark) {
    ...
    final tasks = [
      {'icon': Icons.event_note,        'title': 'Demande de congé',      'cost': 10, 'color': const Color(0xFFA855F7)},
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: isDark ? const Color(0xFF1A1A1A) : Colors.white,
              borderRadius: BorderRadius.circular(20),
              border: Border.all(color: const Color(0xFFA855F7).withOpacity(0.3), width: 1),
            ),
            child: Column(
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Budget Énergie Hera',
                              style: TextStyle(
                                  color: isDark ? Colors.white : Colors.black,
                                  fontSize: 18, fontWeight: FontWeight.bold)),
                          const SizedBox(height: 4),
                          Text('Réinitialisé chaque jour',
                              style: TextStyle(
                                  color: isDark ? Colors.white60 : Colors.black54,
                                  fontSize: 13)),
                        ],
              ...
              Container(
                ...
                child: Text('${tasks.length}',
                    style: const TextStyle(
                        color: Color(0xFFA855F7), fontSize: 12, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Icon(Icons.bolt, size: 14, color: task['color'] as Color),
                                const SizedBox(width: 4),
                                Text('Coût : $cost unités',
                                    style: TextStyle(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          ...
                          style: TextStyle(
                            color: canAfford ? (task['color'] as Color) : Colors.red,
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: const Color(0xFFA855F7).withOpacity(0.2), width: 1),
            ),
            child: Row(
              children: [
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // EMPLOYEES — avec Pull-to-refresh
  // ══════════════════════════════════════════════════════════════════════════

  Widget _buildEmployees(bool isDark) {
    final activeEmployees = _employees.where((e) => e['status'] == 'active').toList();
    final onboardingEmployees = _employees.where((e) => e['status'] == 'onboarding').toList();
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 12),
                    decoration: BoxDecoration(
                      color: _employeeSubTab == 0 ? Colors.white : Colors.transparent,
                      borderRadius: BorderRadius.circular(10),
                      ...
                      children: [
                        ...
                        if (activeEmployees.isNotEmpty) ...[
                          ...
                          Container(
                            ...
                            child: Text('${activeEmployees.length}',
                                style: const TextStyle(
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.symmetric(vertical: 12),
                    decoration: BoxDecoration(
                      color: _employeeSubTab == 1 ? Colors.white : Colors.transparent,
                      borderRadius: BorderRadius.circular(10),
                      ...
                      children: [
                        ...
                        if (onboardingEmployees.isNotEmpty) ...[
                          ...
                          Container(
                            ...
                            child: Text('${onboardingEmployees.length}',
                                style: const TextStyle(
                                    color: Colors.black, fontSize: 11,
                                    fontWeight: FontWeight.bold)),
                          ),
  ...
  Widget _buildActiveEmployeesList(List<Map<String, dynamic>> employees, bool isDark) {
    if (employees.isEmpty) {
      return _buildEmpty(Icons.people_outline, 'Aucun employé actif',
          'Tous vos employés sont en cours d\'intégration', isDark);
    }
    return ListView(
  ...
  Widget _buildOnboardingEmployeesList(List<Map<String, dynamic>> employees, bool isDark) {
    if (employees.isEmpty) {
      return _buildEmpty(Icons.celebration, 'Aucun nouvel arrivant',
          'Tous vos employés sont déjà actifs', isDark);
    }
    return ListView(
      padding: const EdgeInsets.symmetric(horizontal: 16),
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              _badge(Icons.beach_access,
                  '${balances['annual']['remaining']}/${balances['annual']['total']}', isDark),
              const SizedBox(width: 8),
              _badge(Icons.medical_services,
                  '${balances['sick']['remaining']}/${balances['sick']['total']}', isDark),
              const SizedBox(width: 8),
              _badge(Icons.warning_amber_rounded,
                  '${balances['urgent']['remaining']}/${balances['urgent']['total']}', isDark),
            ],
          ),
        ],
      ),
  ...
  Widget _buildOnboardingEmployeeCard(Map<String, dynamic> employee, bool isDark) {
    ...
    if (startDateStr != null && startDateStr.isNotEmpty) {
      try {
        ...
        final daysUntilStart = startDate
                    ...
                    if (countdownText.isNotEmpty)
                      Container(
                        ...
                        child: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            const Icon(Icons.schedule, size: 14, color: Colors.black),
                            const SizedBox(width: 6),
                            Text(countdownText,
                                style: const TextStyle(
                                    color: Colors.black, fontSize: 11,
