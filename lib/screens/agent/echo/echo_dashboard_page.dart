...
class _EchoDashboardPageState extends State<EchoDashboardPage>
  ...
  bool _isLocalInferredTaskId(String id) => id.startsWith('local-');

  List<TaskItem> _dedupeTasksById(List<TaskItem> items) {
    final seen = <String>{};
    final out = <TaskItem>[];
    for (final t in items) {
      final id = t.id.isEmpty
          ? 'empty-${t.title.hashCode}-${t.createdAt.millisecondsSinceEpoch}'
          : t.id;
  ...
  /// Tâches dérivées des e-mails (actions, urgence, mots-clés réunion) si l’API /tasks est vide.
  List<TaskItem> _inferredTasksFromEmails() {
    final out = <TaskItem>[];
    final meetingHint =
        RegExp(r'meeting|meting|réunion|reunion|rdv|rendez-vous|convocation',
            caseSensitive: false);
    for (final e in _emails) {
      if (e.isSpam) continue;
      if (e.sender == 'echo@e-team.com') continue;
      if (e.category == 'auto_reply' || e.category == 'auto_reply_pending') {
        continue;
      }
      var idx = 0;
      for (final action in e.actions) {
        ...
        out.add(TaskItem(
          ...
          title: a,
          description:
              e.subject.isNotEmpty ? e.subject : (e.summary.isNotEmpty ? e.summary : ''),
          category: e.category.isNotEmpty ? e.category : 'message',
          priority: e.priority,
          status: 'todo',
          confidence: 1,
          createdAt: e.receivedAt,
          notes: 'Depuis le message de ${e.sender}',
        ));
        idx++;
      }
      final haystack = '${e.subject} ${e.summary}';
      if (e.actions.isEmpty &&
          (e.isUrgent || meetingHint.hasMatch(haystack))) {
        out.add(TaskItem(
          id: 'local-${e.id}-subj',
          title: e.subject.isNotEmpty ? e.subject : 'Message à traiter',
          description: e.summary.isNotEmpty
              ? e.summary
              : (e.content.length > 200 ? '${e.content.substring(0, 200)}…' : e.content),
          category: e.category.isNotEmpty ? e.category : 'inbox',
          priority: e.priority,
          status: 'todo',
          confidence: 1,
          createdAt: e.receivedAt,
          notes: 'Dérivé du message de ${e.sender}',
        ));
      }
    }
    return out;
  ...
  String _taskStatusBucket(String status) {
  ...
  Map<String, List<TaskItem>> _groupTasksFromFlat(List<TaskItem> tasks) {
    final todo = <TaskItem>[];
    final inProgress = <TaskItem>[];
    final completed = <TaskItem>[];
    for (final t in tasks) {
      final b = _taskStatusBucket(t.status);
      if (b == 'completed') {
        completed.add(t);
  ...
  void _rebucketTasksFromMergedList(List<TaskItem> merged) {
    final grouped = _groupTasksFromFlat(merged);
    _todoTasks = grouped['todo'] ?? [];
    _inProgressTasks = grouped['in_progress'] ?? [];
    _completedTasks = grouped['completed'] ?? [];
    _overdueTasks = merged.where((t) => t.isOverdue).toList();
    _taskStats = TaskStats(
      todo: _todoTasks.length,
      inProgress: _inProgressTasks.length,
      completed: _completedTasks.length,
      overdue: _overdueTasks.length,
    );
  }

  void _patchLocalTask(String taskId, String newStatus) {
    ...
    _rebucketTasksFromMergedList(_tasks);
  }
  ...
  Future<void> _loadTasks() async {
    ...
    try {
      ...
      if (!mounted) return;
      final inferred = _inferredTasksFromEmails();
      final merged = _dedupeTasksById([...result.tasks, ...inferred]);
      final grouped = _groupTasksFromFlat(merged);
      final todo = grouped['todo'] ?? [];
      final inProg = grouped['in_progress'] ?? [];
      final done = grouped['completed'] ?? [];
      final overdue = merged.where((t) => t.isOverdue).toList();
      ...
      if (!mounted) return;
      final inferred = _inferredTasksFromEmails();
      final grouped = _groupTasksFromFlat(inferred);
      setState(() {
        _tasks = inferred;
        _todoTasks = grouped['todo'] ?? [];
        _inProgressTasks = grouped['in_progress'] ?? [];
        _completedTasks = grouped['completed'] ?? [];
        _overdueTasks = inferred.where((t) => t.isOverdue).toList();
        _taskStats = TaskStats(
          todo: _todoTasks.length,
          inProgress: _inProgressTasks.length,
          completed: _completedTasks.length,
  ...
  Future<void> _updateTaskStatus(String taskId, String newStatus) async {
    if (_isLocalInferredTaskId(taskId)) {
      if (!mounted) return;
      setState(() => _patchLocalTask(taskId, newStatus));
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ Tâche mise à jour (locale)'),
  ...
  Future<void> _deleteTask(String taskId) async {
    if (_isLocalInferredTaskId(taskId)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ Tâche retirée de la liste'),
          backgroundColor: Colors.green,
        ),
      );
      return;
  ...
  String _emailListSummaryLine(EmailItem email, bool isAutoReply) {
    final s = email.summary;
    if (s.isEmpty) return s;
    if (isAutoReply) {
      final lower = s.toLowerCase();
      if (lower.contains('envoi dans') ||
          lower.contains('réponse automatique') ||
          lower.contains('reponse automatique')) {
  ...
  Widget _buildEmailCard(EmailItem email, bool isDark) {
    ...
    final listSummary = _emailListSummaryLine(email, isAutoReply);
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(listSummary.length > 100 ? '${listSummary.substring(0, 100)}...' : listSummary, style: TextStyle(fontSize: 12, color: isDark ? Colors.white60 : Colors.grey[600]), maxLines: 2, overflow: TextOverflow.ellipsis),
